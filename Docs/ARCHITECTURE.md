...
| **P5 战斗原型** | 🔄 **STEP 1 / STEP 2 已落成**（2026-09-14，细分见下） | 3×3 棋盘、自动战斗、五行结算（STEP 1）+ 宿主×灵魂融合管线（STEP 2） | Luban（独立命令行工具，非 UPM 包） | 灰盒下连看 10 场不无聊（人工判据，自检只备料） |
| **P6 业务模块** | ⬜ | 图鉴、肉鸽地图、设置等（融合已在 P5 STEP 2 提前落地） | — | 一局完整通关 35-50 分钟 |

**P5 的落地细分（对照 git 历史）：**

| STEP | 内容 | 状态 | 验收判据 |
|------|------|------|---------|
| STEP 1 | 棋盘战斗逻辑闭环：九宫格 / 出手序列 / CD 与技能 / 五行系数 / 相邻格 / 羁绊 / 胜负 + 视图帧流 + 灰盒窗口 | ✅ `battle.selftest` 14 项全绿 | ① 同种子 100% 可复现（指纹比对）✅ ③ 改系数立即可观察（自检读 GDD 2.3 DocTable 逐格比对）✅ ② 灰盒 10 场不无聊 = **人工判断** |
| STEP 2 | 宿主×灵魂融合管线：`SoulDef` / `FusionRules`（融合算法 + 色相偏移）/ `SoulForge`（异兽→灵魂派生）/ `FusionNaming` + **30 只异兽内容导入**（beasts.json → 配置资产）+ 融合预览窗口 | ✅ `fusion.selftest` 20 项（离线冒烟 `_tools/fusion-smoke` 同口径全绿） | ① 3×3=9 组合外观与技能组均不重复 ✅ ② 融合单位战斗行为正确（五行覆写 1.50→0.75、灵魂战技 20/20 局真实施放、特性叠加）✅ ③ 切灵魂实时变色（数据层 ✅ + 预览窗口人工确认） |
