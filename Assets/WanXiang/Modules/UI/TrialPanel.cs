                ...
                case 0:  // 登天阙：**进入天阙图**（休整→商店→熔炼→看护关→后土），不是直接开战
                    {
                        ...
                        // ★★ 现在天阙图是【就地】生成的（CampaignPanel 没切场景），
                        //    本面板关掉后自然回到下面的节点图 ⇒ 什么都不用切。
                        var run5 = WanXiang.Run.RunSave.Current;
                        if (run5 != null)
                        {
                            run5.Act = 5;
                            run5.NodeOffset = -1;
                            if (run5.VisitedNodes != null) run5.VisitedNodes.Clear();
                            WanXiang.Run.RunSave.SaveCurrent();
                        }
                        CloseSelf();
                    }
                ...
                default:  // 归元：结束本局，回主城
                    {
                        ...
                        catch (System.Exception ex) { Debug.LogWarning("[TrialPanel] 局外结算异常：" + ex.Message); }
                        if (runEnd != null)
                        {
                            runEnd.Act = 1;
                            runEnd.NodeOffset = -1;
